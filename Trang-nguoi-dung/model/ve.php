...
function loadone_vephim($id){
    $sql="SELECT v.id, v.id_tk, phim.tieu_de,lichchieu.ngay_chieu, v.price, v.ngay_dat, v.ghe, v.combo, taikhoan.name, khung_gio_chieu.thoi_gian_chieu, v.id_hd, v.trang_thai, phongchieu.name as tenphong
    FROM ve v
    LEFT JOIN taikhoan ON taikhoan.id = v.id_tk
    LEFT JOIN khung_gio_chieu ON khung_gio_chieu.id = v.id_thoi_gian_chieu
    LEFT JOIN phim ON phim.id = v.id_phim
...
function huy_vephim($id) {
    $sql = "UPDATE ve SET trang_thai = 3 WHERE id = $id";
    pdo_execute($sql);
}
