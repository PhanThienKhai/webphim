<?php
// FIX: Set UTF-8 charset để tránh lỗi phông chữ khi deploy
header('Content-Type: text/html; charset=utf-8');

// FIX: Hiển thị errors khi deploy để debug
        ...
        switch ($act) {
            ...
            case "nv_chamcong_face":
                ...
                if (!isset($_SESSION['user1']) || !isset($_SESSION['user1']['id'])) {
                    ...
                    echo "<p>Vui lòng <a href='/Trang-admin/login.php'>đăng nhập</a> lại.</p>";
                    exit;
                }
            ...
            case "duyet_lichchieu":
                ...
                // FIX: Lấy danh sách lịch chiếu nhóm theo mã kế hoạch - chỉ show kế hoạch của cum này
                $id_cum = $_SESSION['user1']['id_cum'] ?? null;
                $ds_lich = lc_list_grouped_for_approval($filter, $id_cum);
                
                include "./view/cum/duyet_lich.php";
                break;
            case "bulk_duyet_lichchieu":
                // AJAX endpoint for bulk approval
                header('Content-Type: application/json; charset=utf-8');
                
                $action = $_POST['action'] ?? ''; // 'duyet' or 'tu_choi'
                $ma_ke_hoach_list = $_POST['ma_ke_hoach_list'] ?? []; // array of ma_ke_hoach
                ...
                if (!in_array($action, ['duyet', 'tu_choi'])) {
                    $response['message'] = 'Hành động không hợp lệ';
                    echo json_encode($response);
                    exit;
                }
                
                if (!is_array($ma_ke_hoach_list) || empty($ma_ke_hoach_list)) {
                    $response['message'] = 'Không có kế hoạch được chọn';
                    echo json_encode($response);
                    exit;
                }
                
                // Validate mã kế hoạch and sanitize
                $valid_ma_ke_hoach = [];
                foreach ($ma_ke_hoach_list as $ma) {
                    if (is_string($ma) && strlen($ma) > 0) {
                        $valid_ma_ke_hoach[] = $ma;
                    }
                }
                
                if (empty($valid_ma_ke_hoach)) {
                    $response['message'] = 'Dữ liệu kế hoạch không hợp lệ';
                    echo json_encode($response);
                    exit;
                }
                ...
                foreach ($valid_ma_ke_hoach as $ma_ke_hoach) {
                    $sql = "UPDATE lichchieu SET trang_thai_duyet = ? WHERE ma_ke_hoach = ?";
                    pdo_execute($sql, $trang_thai, $ma_ke_hoach);
                    $count++;
                }
                ...
                $response['message'] = ($action === 'duyet') ? 'Duyệt thành công' : 'Từ chối thành công';
