...
function cc_register_face($id_nv, $fingerprint_json) {
    ...
    // FIX: Use PHP date() instead of MySQL NOW() to ensure correct timezone
    $now = date('Y-m-d H:i:s');
    pdo_execute("UPDATE taikhoan SET face_template = ?, face_registered_at = ? WHERE id = ?",
                $fingerprint_json, $now, $id_nv);
