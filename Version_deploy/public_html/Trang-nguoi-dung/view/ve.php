...
if (file_exists(__DIR__ . '/../config/qr_config.php')) {
    ...
    if (!empty(QR_SERVER_IP)) {
        ...
        if (QR_SERVER_PORT != 80 && QR_SERVER_PORT != 443) {
...
<section class="container">
    <div class="order-container">
        <?php
        // FIX: Kiểm tra xem $load_ve có tồn tại và có dữ liệu không
        if (!isset($load_ve)) {
            echo '<div class="alert alert-warning">Không tìm thấy vé. ID: ' . htmlspecialchars($_GET['id'] ?? 'N/A') . '</div>';
            $load_ve = [];
        } elseif (!is_array($load_ve)) {
            echo '<div class="alert alert-danger">Lỗi: Dữ liệu vé không hợp lệ</div>';
            $load_ve = [];
        }
                            ...
                            <div class="ticket__inner">
                                ...
                                <div class="ticket-primery" style="position: relative;">
                                    <div style="position: absolute; top: 86px; right: -1px; width: 107px; height: 107px; background: #fff; border: 2px solid #e5e7eb; border-radius: 8px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                                        <img src="view/qr.php?data=' . urlencode("https://" . $_SERVER['HTTP_HOST'] . "/Trang-nguoi-dung/quete.php?id=" . $id) . '&t=' . time() . '" alt="QR Code" style="width: 115px; height: 115px; object-fit: contain;" />
                                    </div>
<span class="ticket__item ticket__item--primery ticket__film" style="display:flex;"> <strong class="ticket__movie" >PHIM : ' . $tieu_de . '</strong></span>                                    <span class="ticket__item ticket__item--primery">🪑 Ghế: <span class="ticket__place">' . $ghe . '</span></span>
            ...
            echo '<div class="alert alert-info" style="padding: 20px; text-align: center; color: #0891b2;">
                    <h3>📭 Bạn chưa có vé nào</h3>
                    <p>Vui lòng <a href="index.php">mua vé</a> để xem chi tiết</p>
                  </div>';
        }
        ?>
    </div>
